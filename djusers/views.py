...
def tree_view(request):
    ...
    r=100*data2.productinfo/204
    s=r-.15*r
    payout=s*data2.no_of_referals+s*p_data
...
def profile_view(request):
	...
	package = 100*data.productinfo/102
	context={'data':data, 'package':package}
	if  request.method == 'POST':
...
def edit_profile_view(request):
	...
	if request.method == 'POST':
		if 'Name_of_account_holder' in request.POST:
			...
			up.uidai = request.POST['uidai']
			up.pan = request.POST['pan']
			up.save()
			success=True
			context={
